  ...
  return (
    <Container fixed style={{ padding: "4%", backgroundColor: "#D9D9D9" }}>
      <h4 style={{ fontSize: 20 }}>
        ISXX게시판: ISXX에 대해 자유롭게 이야기를 나눠주세요!
      </h4>
      {posts !== null ? (
        <StBoard>
          <div key={posts.id}>
          ...
          <div className="Username" style={{ marginBottom: "3%" }}>
            닉네임: {posts.nickname} {posts.createAt}
          </div>
          <div
            style={{ padding: "2%", backgroundColor: "#D9D9D9" }}
            className="Content"
          >
            {posts.content}
          </div>
          <div className="buttons" style={{ float: "right", margin: "3%" }}>
            <Button onClick={() => navigate(`/${boardname}/editposts/${id}`)}>
              수정
            </Button>
            <Button
              onClick={() => {
